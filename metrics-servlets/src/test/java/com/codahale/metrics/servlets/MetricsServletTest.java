...
public class MetricsServletTest extends AbstractServletTest {
	...
	public void setUp() throws Exception {
		...
		registry.register("t",
				new Timer(new ExponentiallyDecayingReservoir(), clock)).update(
				1, TimeUnit.SECONDS);
		...
		assertThat(response.getContent())
				.isEqualTo(
						"{"
								...
								+ "}" + "}");
		assertThat(response.get(HttpHeader.CONTENT_TYPE)).isEqualTo(
				"application/json");
	}
	...
	public void optionallyPrettyPrintsTheJson() throws Exception {
		...
		assertThat(response.getContent()).isEqualTo(
				String.format("{%n" + "  \"version\" : \"3.0.0\",%n"
						+ "  \"gauges\" : {%n" + "    \"g1\" : {%n"
						+ "      \"value\" : 100%n" + "    }%n" + "  },%n"
						+ "  \"counters\" : {%n" + "    \"c\" : {%n"
						+ "      \"count\" : 1%n" + "    }%n" + "  },%n"
	...
	public void filterMetrics() throws Exception {
		...
		assertThat(response.get("Access-Control-Allow-Origin")).isEqualTo("*");
		assertThat(response.getContent()).isEqualTo(
				String.format("{%n" + "  \"count\" : 1,%n"
						+ "  \"m15_rate\" : 0.0,%n" + "  \"m1_rate\" : 0.0,%n"
						+ "  \"m5_rate\" : 0.0,%n"
						+ "  \"mean_rate\" : 3333333.3333333335,%n"
	...
	public void constructorWithRegistryAsArgumentIsUsedInPreferenceOverServletConfig()
		...
		verify(servletContext, never()).getAttribute(
				eq(MetricsServlet.METRICS_REGISTRY));
	}

	@Test
	public void constructorWithRegistryAsArgumentUsesServletConfigWhenNull()
			throws Exception {
		final MetricRegistry metricRegistry = mock(MetricRegistry.class);
		final ServletContext servletContext = mock(ServletContext.class);
		final ServletConfig servletConfig = mock(ServletConfig.class);
		...
		verify(servletConfig, times(1)).getServletContext();
		verify(servletContext, times(1)).getAttribute(
				eq(MetricsServlet.METRICS_REGISTRY));
	}
