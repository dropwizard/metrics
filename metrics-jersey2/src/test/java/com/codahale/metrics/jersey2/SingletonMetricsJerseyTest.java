...
/**
 * Tests registering {@link MetricsApplicationEventListener} as a singleton
...
public class SingletonMetricsJerseyTest extends JerseyTest {
    ...
    public void timedMethodsAreTimed() {
        assertThat(target().path("timed").request().get(String.class)).isEqualTo("yay");
    ...
    public void meteredMethodsAreMetered() {
        assertThat(target().path("metered").request().get(String.class)).isEqualTo("woo");
    ...
    public void exceptionMeteredMethodsAreExceptionMetered() {
        ...
        assertThat(target().path("exception-metered").request().get(String.class)).isEqualTo("fuh");

        assertThat(meter.getCount()).isZero();

        try {
            target().path("exception-metered").queryParam("splode", "true").request().get(String.class);
