...
/**
 * A reporter which publishes Kibana-friendly metric values to an Elasticsearch cluster 
 ...
 * @see <a href="http://www.elasticsearch.org/guide/en/elasticsearch/reference/current/modules-http.html">Elasticsearch HTTP API</a>
 */
public class ElasticsearchHttpReporter extends ElasticsearchReporter {
     ...
     * @param elasticsearchIndexPrefix the indices created will be prefixed with this value, e.g. "test" will result with indices like "test-2014.08.10"
     * @return a {@link Builder} instance for a {@link ElasticsearchHttpReporter}
     */
    public static Builder forRegistryAndIndexPrefix(MetricRegistry registry, String elasticsearchIndexPrefix) {
        return new Builder(registry, elasticsearchIndexPrefix);
    }

    /**
     * A builder for {@link ElasticsearchHttpReporter} instances. Defaults to not using a prefix, using the
    ...
    public static class Builder {
         ...
         * 
         * @param bulkRequestLimit The number of documents per request, defaults to 2500
         * @return {@code this}
         */
         ...
         * @param timestampFieldName The field name to log the timestamp to
         * @return {@code this}
         */
        ...
        /**
         * Builds a {@link ElasticsearchHttpReporter} with the given properties and the given hosts. 
         * The reporter will round-robin through each host for sending metrics. 
         *
         * @param elasticsearchHosts The Elasticsearch hosts to send metrics to
        ...
        public ElasticsearchHttpReporter build(String...elasticsearchHosts) {
            return new ElasticsearchHttpReporter(registry,
                                        ...
                                        durationUnit,
                                        filter, 
                                        timeout, bulkRequestLimit, elasticsearchHosts);
        }
    }
	...
	protected ElasticsearchHttpReporter(MetricRegistry registry, Clock clock, String elasticsearchIndexPrefix, String timestampFieldName,
			String metricPrefix, TimeUnit rateUnit, TimeUnit durationUnit, MetricFilter filter, int timeout, int bulkRequestLimit, String...elasticsearchHosts) {
		super(registry, clock, elasticsearchIndexPrefix, timestampFieldName, metricPrefix, rateUnit, durationUnit, filter);
	...
	@Override
	protected void addReportToBulkRequest(String index, String type, String json) throws IOException {
    ...
    /**
     * Open a new HttpUrlConnection to the Elasticsearch cluster
     * This operates in a round robin fashion to load balance reporting
     */
    private HttpURLConnection openConnection(String uri, String method) {
		int hostIndex = nextHost.get();
        nextHost.set(nextHost.get() == elasticsearchHosts.length - 1 ? 0 : nextHost.get() + 1);
    	try {
            URL templateUrl = new URL("http://" + elasticsearchHosts[hostIndex]  + uri);
            ...
            LOGGER.error("Error connecting to {}: {}", elasticsearchHosts[hostIndex], e);
        }
        return null;
    }
